            ...
            if ($get_auto_apply_candidate) {
                foreach ($get_auto_apply_candidate as $get_auto_apply_candidate) {
                    ...
                    if ($check_job) {
                        ...
                        $apply->save();
                        $get_recruiter_contact = Company::select('users.mobile')->Join('jobs', 'jobs.company_id', '=', 'companies.id')->Join('users', 'users.id', '=', 'companies.user_id')->where('jobs.id', $request->job_id)->first();
